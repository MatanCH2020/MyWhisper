...
| 📋 **היסטוריית העתקות** | כל טקסט/תמונה שהעתקת נשמר — ‎``Ctrl+` ``‎ פותח רשימה עם חיפוש. סיסמאות ממנהלי סיסמאות לא נשמרות |
...
| `clipboard_history` | שמירת כל מה שמעתיקים (ברירת מחדל `true`). כיבוי מבטל גם את המעקב וגם את הקיצור |
| `clipboard_hotkey` | הקיצור שפותח את רשימת ההעתקות (ברירת מחדל `` ctrl+` ``). שימו לב: ב-VS Code זה קיצור הטרמינל, ו-MyWhisper גובר עליו — שנו כאן אם זה מפריע |
| `clipboard_paused` | השהיית שמירת ההעתקות (נשלט גם ממסך ההגדרות) |
